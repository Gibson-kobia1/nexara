// api/update-connection.ts
import { createClient } from '@supabase/supabase-js';
export default async function handler(req: any, res: any) {
  console.log("Update API called with body:", req.body);
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }
  const { tracking_id, confirmation_link, code } = req.body || {};
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error("Service role key missing. Available env keys:", Object.keys(process.env));
